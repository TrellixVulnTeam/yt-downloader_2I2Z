...
def from_query(request: str, output_format: str = None):
    global playlistId, ytmusic, is_playlist
    songObjList = []
    if "open.spotify.com" in request and "track" in request:
        ...
        try:
            # Create playlist
            playlistId = ytmusic.create_playlist("spotify", "conversion from spotify", "PUBLIC")
            is_playlist = 1
...
def songobj_from_spotify_url(spotifyURL: str, output_format: str = None):
    ...
    if youtubeLink is None:
        ...
        try:
            url_data = urlparse.urlparse(youtubeLink)
            query = urlparse.parse_qs(url_data.query)
            video = query["v"][0]
            print(video)
            ytmusic.add_playlist_items(playlistId, [video])
        except:
            pass
